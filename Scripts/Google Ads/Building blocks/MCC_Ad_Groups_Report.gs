// --- MCC AD GROUPS REPORT CONFIGURATION ---
...
function main() {
  Logger.log("=== Starting MCC Ad Groups Report ===");
  ...
  for (const mapping of accountMappings) {
    ...
    try {
      ...
      // Get ad groups data for this account
      const adGroupsData = getAdGroupsDataForAccount();
      
      // Update the account's spreadsheet
      updateAccountSpreadsheet(spreadsheetUrl, adGroupsData, accountName);
      
      Logger.log(`✓ Successfully updated ad groups data for ${accountName}`);
      successCount++;
  ...
  // Summary
  Logger.log(`\n=== MCC AD GROUPS REPORT COMPLETED ===`);
...
function getAdGroupsDataForAccount() {
  Logger.log("Getting ad groups data for current account...");
  ...
  try {
    ...
    Logger.log(`✓ Processed ${data.length} ad groups for current account`);
    return sortedData;
    
  } catch (error) {
    Logger.log(`❌ Error getting ad groups data: ${error.message}`);
    throw error;
  }
}
...
function calculateMetrics(rows) {
  ...
  while (rows.hasNext()) {
    try {
      ...
      // Skip ad groups with zero impressions
      if (impressions === 0) {
        continue;
      }
...
function updateAccountSpreadsheet(spreadsheetUrl, adGroupsData, accountName) {
  try {
    Logger.log("Updating account spreadsheet with ad groups data...");
    
    const spreadsheet = SpreadsheetApp.openByUrl(spreadsheetUrl);
    Logger.log(`✓ Opened spreadsheet: ${spreadsheet.getName()}`);
    
    // Create or get the tab for ad groups data
    ...
    if (adGroupsData.length > 0) {
      allData = allData.concat(adGroupsData);
    } else {
      // Add a row indicating no data found
      allData.push(['No ad groups data found for the last 30 days']);
    }
    ...
    Logger.log(`✓ Successfully updated ${TAB} sheet with ${adGroupsData.length} ad groups`);

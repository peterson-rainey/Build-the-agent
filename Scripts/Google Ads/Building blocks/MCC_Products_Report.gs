// --- MCC PRODUCTS REPORT CONFIGURATION ---
...
function main() {
  Logger.log("=== Starting MCC Products Report ===");
  ...
  for (const mapping of accountMappings) {
    ...
    try {
      ...
      // Get products data for this account
      const productsData = getProductsDataForAccount();
      
      // Update the account's spreadsheet
      updateAccountSpreadsheet(spreadsheetUrl, productsData, accountName);
      
      Logger.log(`✓ Successfully updated products data for ${accountName}`);
      successCount++;
...
function getProductsDataForAccount() {
  Logger.log("Getting products data for current account...");
  ...
  try {
    ...
    Logger.log(`✓ Processed ${data.length} products for current account`);
    return sortedData;
    
  } catch (error) {
    Logger.log(`❌ Error getting products data: ${error.message}`);
    throw error;
  }
}
...
function calculateMetrics(rows) {
  ...
  while (rows.hasNext()) {
    try {
      ...
      // Access product segments
      let productItemId = row.segments ? row.segments.productItemId : 'N/A';
      let productTitle = row.segments ? row.segments.productTitle : 'N/A';
      let productTypeL1 = row.segments ? row.segments.productTypeL1 : 'N/A';
      let productTypeL2 = row.segments ? row.segments.productTypeL2 : 'N/A';
      let productTypeL3 = row.segments ? row.segments.productTypeL3 : 'N/A';
      let productTypeL4 = row.segments ? row.segments.productTypeL4 : 'N/A';
      let productTypeL5 = row.segments ? row.segments.productTypeL5 : 'N/A';

      // Determine combined status - if either is paused, show as paused
      ...
      // Skip products with zero impressions
      if (impressions === 0) {
        continue;
      }

      // Add product data to a new row
      let newRow = [
        campaignName,
        campaignType,
        adGroupName,
...
function sortData(data) {
  return data.sort((a, b) => {
    ...
    const conversionsA = a[14] || 0; // conversions is at index 14
    const conversionsB = b[14] || 0;
...
function updateAccountSpreadsheet(spreadsheetUrl, productsData, accountName) {
  try {
    Logger.log("Updating account spreadsheet with products data...");
    
    const spreadsheet = SpreadsheetApp.openByUrl(spreadsheetUrl);
    Logger.log(`✓ Opened spreadsheet: ${spreadsheet.getName()}`);
    
    // Create or get the tab for products data
    ...
    if (productsData.length > 0) {
      ...
      const noDataRow = ['No products data found for the last 30 days'];
    ...
    Logger.log(`✓ Successfully updated ${TAB} sheet with ${productsData.length} products`);

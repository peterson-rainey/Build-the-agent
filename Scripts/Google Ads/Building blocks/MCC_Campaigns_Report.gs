// --- MCC CAMPAIGNS REPORT CONFIGURATION ---
...
function main() {
  Logger.log("=== Starting MCC Campaigns Report ===");
  ...
  for (const mapping of accountMappings) {
    ...
    try {
      ...
      // Get campaigns data for this account
      const campaignsData = getCampaignsDataForAccount();
      
      // Update the account's spreadsheet
      updateAccountSpreadsheet(spreadsheetUrl, campaignsData, accountName);
      
      Logger.log(`✓ Successfully updated campaigns data for ${accountName}`);
      successCount++;
  ...
  // Summary
  Logger.log(`\n=== MCC CAMPAIGNS REPORT COMPLETED ===`);
...
function getCampaignsDataForAccount() {
  Logger.log("Getting campaigns data for current account...");
  ...
  try {
    ...
    Logger.log(`✓ Processed ${data.length} campaigns for current account`);
    return sortedData;
    
  } catch (error) {
    Logger.log(`❌ Error getting campaigns data: ${error.message}`);
    throw error;
  }
}
...
function calculateMetrics(rows) {
  ...
  while (rows.hasNext()) {
    try {
      ...
      // Skip campaigns with zero impressions
      if (impressions === 0) {
        continue;
      }
...
function updateAccountSpreadsheet(spreadsheetUrl, campaignsData, accountName) {
  try {
    Logger.log("Updating account spreadsheet with campaigns data...");
    
    const spreadsheet = SpreadsheetApp.openByUrl(spreadsheetUrl);
    Logger.log(`✓ Opened spreadsheet: ${spreadsheet.getName()}`);
    
    // Create or get the tab for campaigns data
    ...
    if (campaignsData.length > 0) {
      allData = allData.concat(campaignsData);
    } else {
      // Add a row indicating no data found
      allData.push(['No campaigns data found for the last 30 days']);
    }
    ...
    Logger.log(`✓ Successfully updated ${TAB} sheet with ${campaignsData.length} campaigns`);
